...
resource "aws_api_gateway_integration" "event" {
  ...
  type                    = "AWS_PROXY"
  uri                     = aws_lambda_function.event_general_api.invoke_arn
}
...
resource "aws_lambda_permission" "event" {
  ...
  function_name = aws_lambda_function.event_general_api.function_name
  principal     = "apigateway.amazonaws.com"
  source_arn    = "${data.aws_api_gateway_rest_api.event_planr.execution_arn}/*/*/event"
}

resource "aws_lambda_permission" "event_proxy" {
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.event_general_api.function_name
  principal     = "apigateway.amazonaws.com"
  source_arn    = "${data.aws_api_gateway_rest_api.event_planr.execution_arn}/*/*/event/*"
